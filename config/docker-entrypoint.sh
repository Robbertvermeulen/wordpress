...
if [ ${#html[@]} -gt 0 ]; then
	...
	openssl req -new -x509 -nodes -key rootca.insecure.key -sha256 -out cacert.pem -days 3650 -subj "/C=GB/ST=London/L=London/O=localhost/OU=IT Department/CN=${SSL_SITE_NAME}"  && \
...
# USER & GROUP CHANGED FROM WWW-DATA TO WORDPRESS
if [[ "$1" == apache2* ]] || [ "$1" == php-fpm ]; then
	if [ "$(id -u)" = '0' ]; then
		case "$1" in
			apache2*)
				user="${APACHE_RUN_USER:-wordpress}"
				group="${APACHE_RUN_GROUP:-wordpress}"
